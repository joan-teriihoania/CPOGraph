...
public class IncidenceArrayGraph implements Graph, Serializable {
  ...
  /**
   * Builds an {@link IncidenceArrayGraph} with a maximum size of n {@link graph.Vertex} and 2n {@link graph.Edge}
   * @param n Maximum size of {@link graph.Vertex}
  */
  ...
  */
  ...
  public String toString(){
    String toReturn = "";
    for(int i = 0;i < this.incidenceArray.length;i++){
      if(this.incidenceArray[i] != null){
        toReturn += i + ": {";
        for(int j = 0;j < this.incidenceArray[i].length;j++){
          if(this.incidenceArray[i][j] != null){
            Vertex[] tmp = this.incidenceArray[i][j].getEnds();
            toReturn += "  ("+tmp[0].getId()+", "+tmp[1].getId()+"),";
          }
        }
        toReturn += "}\n";
      }
  ...
  public static void serialize(String filename, IncidenceArrayGraph incidenceArray){    
    // Serialization 
    try(
      ...
      ObjectOutputStream out = new ObjectOutputStream(file);
        ...
        out.writeObject(incidenceArray);
      ...
      try{
        ...
        System.out.println("A problem occured while trying to create the file");
      }
      serialize(filename, incidenceArray);
    }
  ...
  public static IncidenceArrayGraph deserialize(String filename){
    IncidenceArrayGraph object1 = null;
    ...
    try{
      ...
      object1 = (IncidenceArrayGraph)in.readObject();
